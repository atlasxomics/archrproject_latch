...
def main():
    parser = argparse.ArgumentParser(description="Custom script with command-line parameters")
    parser.add_argument("-c", "--copy", required=True, help="Specify the LATCH URI to copy")
    #parser.add_argument("-a", "--activate", default="", help="Optional activation command")
    parser.add_argument("-a", "--activate", action="store_true", help="Activate the command (without this, it's just a practice run)")
    parser.add_argument("-l", "--location", default="/root/", help="Optional location argument")
    ...
    if not args.activate:
        ...
        NOT be activated.\n*\tAdd the -a or --activate to cmd line.\n**********\n")
    ## destination folder
    ...
    if not fileCopiedAlready:
        ...
        print(f"\n\n!!Directory already exists [{topCopiedPath}]. Remove if you want to re-copy.\n")
    ...
    if len(files) == 1:
        ...
        serviceFile = (f"#!/usr/bin/env Rscript\n\n"
        f"#Need to specify lib that Rstudio is using as it's different from R on commandline\n"
        f".libPaths('/usr/local/lib/R/site-library');"
        f"shiny::runApp('{shinyDir}', "
        f"port=5000, host='0.0.0.0')\n")
        ...
        print(">>DONE\n")
        print(f"Backing up existing file [{customAppPath}] to /root/custom_app.old.txt")
        ...
        if args.activate:
            ...
            os.system(startCmd)
            print(">>DONE\n")
            print("!! You can check to see if service is active by running: >systemctl --user status latch-custom-app on the command line")
        else:
            print("****App was not activated. Run with -a or --activate on commandline.****\n\n")
    else:
        print(f"Could not find obvious ShinyApp directory containing the 'ui.R' file")
        for file in files:
            print(f'Found: {file}')

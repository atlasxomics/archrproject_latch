...
scDRcell <- function(
  ...
  if (is.na(inpConf[UI == inp1]$fCL)) {
    ...
    ggOut <- ggOut +
      ...
      guides(
        color = guide_legend(
          override.aes = list(size = 5), nrow = inpConf[UI == inp1]$fRow
        )
      ) +
...
scDRcoex <- function(
  ...
  pbmc <- AddModuleScore(
    combined, features = list(signature = geneList$gene), name = "signature"
  )
  reduction <- substring(inpdrX, 1, nchar(inpdrX) - 2)
...
Creat_matrix <-  function(seMarker) {
  ...
  for (an in assayNames) {
    eval(
      parse(
        text = paste0(
          an, " <- ", "SummarizedExperiment::assays(seMarker)[['", an, "']]"
        )
      )
    )
  }
  ...
  if (ncol(mat) == 1) {
    ...
    idx <- which(
      rowSums(passMat, na.rm = TRUE) > 0 &
      matrixStats::rowVars(mat) != 0 &
      !is.na(matrixStats::rowVars(mat))
    )
  }
  subsetMarkers = NULL
  if(!is.null(subsetMarkers)) {
    if(length(which(subsetMarkers %ni% 1:nrow(mat))) == 0){
      ...
      stop(
        paste0(
          "Rownames / indices provided to the subsetMarker parameter",
          "are outside of the boundaries of seMarker."
        )
      )
    }
...
scBubbHeat <- function(
  ...
  for(iGene in geneList$gene){
    temp = as.data.frame(
      t(
        setDT(cmplxData)[
          order(-(geneName==iGene)), .SD[1L] ,.(grpBy)
        ][, c("geneName", "val")]
      )
    )[-c(1),]
    colnames(temp) <- as.character(
      cmplxData[which(cmplxData$geneName==iGene),]$grpBy
    )
  ...
  cmplxData3 <- cmplxData3[
    , order(as.numeric(gsub("C", "", colnames(cmplxData3))))
  ]
...
.binarySort <- function(
  ...
  rowIdx <- do.call(
    "order",
    c(as.data.frame(lmat)[seq_len(ncol(lmat))],
...
Creat_matrix_motif <- function(
  ...
  if (nrow(mat2) > 1 & ncol(mat2) > 1) {
    if (binaryClusterRows) {
      bS <- .binarySort(
        mat2,
        lmat = passMat[rownames(mat2), colnames(mat2)],
...
scBubbHeat2 <- function(
  ...
  if(inpGrp == "Clusters"){
    ...
    out <- mapply(function(x, y) S4Vectors::DataFrame(
      geneName = rep(x[, 1], y), val = rep(x[, 2],y)
    ), d, n1)
    ...
    out <- lapply(out, function(x) S4Vectors::DataFrame(
  ...
  shiny::validate(
    need(uniqueN(ggData$grpBy) > 1, "Only 1 group present, unable to plot!")
  )
  colRange = c(-max(abs(range(ggData$val))), max(abs(range(ggData$val))))
  ...
  for (iGene in geneList$gene) {
    temp = as.data.frame(
      t(
        setDT(cmplxData)[
          order(-(geneName == iGene)), .SD[1L], .(grpBy)][, c("geneName", "val")
        ]
      )
    )[-c(1),]
    colnames(temp)= as.character(
...
shinyServer(function(input, output, session) { 
  ...
  grps <- names(getCellColData(proj))[grep(
    "condition_", names(getCellColData(proj))
  )]
  ...
  observeEvent(
    input$sc1de1grp,
    {
      selected_markers <- try(
        sort(
          unique(getCellColData(proj)[input$sc1de1grp][, 1])
        ),
        silent = TRUE
      )
      updateSelectizeInput(
        session,
        inputId = "sc1de1subgrp",
        choices = selected_markers,
        selected = selected_markers[1]
      )
    }
  )
  ...
  scvolcano <- function(grp) {
    ...
    volcplot <- function(inpMarkers) {           
      ...
      cond1 = unique(
        ggData[
          which(
            ggData$avg_log2FC > 0 & ggData$Significance != "Not siginficant"
          ), 
        ]$Significance
      )    
      others = unique(
      ...
      if(nrow(ggData) == 0) {
        ...
        showNotification(
          paste0(
            "There is no volcano plot for cluster ",
            input$sc1de1inp,
            "because it contains more than 90% from one conditions. Please ",
        ...
        minfdr <- 0.09
        minfdr1 <- 10^-(
          1/6 * (-log10(min(inpMarkers[cluster == input$sc1de1inp]$p_val_adj)))
        )
        minfdr2 <- 10^-(
          2/3 *(-log10(min(inpMarkers[cluster == input$sc1de1inp]$p_val_adj)))
        )
        ...
        ggData$Significance <- factor(
          ggData$Significance,
          levels = c(cond1, others, "Not siginficant")
        )
  ...
  output$sc1de1oup.pdf <- downloadHandler(
    filename = function() {
      "sc1_volcano.pdf"
    },
    content = function(file) {
      ggsave(
        file,
        device = "pdf",
        height = input$sc1de1oup.h,
        width = input$sc1de1oup.w,
        useDingbats = FALSE,
        plot = scvolcano(input$sc1de1grp)
      )
    }
  )
  output$sc1de1oup.png <- downloadHandler(
    filename = function() {
      "sc1_volcano.png"
    },
    content = function(file) {
  ...
  ### Plots for tab a1Metadata vs geneAccs #####################################
  ...
  observeEvent(
    ...
    {
      sub = strsplit(
        sc1conf[UI == input$sc1a1sub1]$fID, "\\|")[[1]]
  ...
  dcn <- reactive(
    {
      strsplit(sc1conf[UI == input$sc1a1inp1]$fID,
      "\\|")[[1]]
    }
  )
      ...
      if (file.exists(paste0("colorset_", input$sc1a1inp1, ".csv")) == TRUE) {
        ...
        choices <- unique(
          grep(
            ...
            colnames(getCellColData(ArchRProj = proj)),
            value = TRUE
          )
        )
        for (choice in choices) {
          write.csv(
            colorRampPalette(brewer.pal(12, "Paired"))(length(unique(
              getCellColData(ArchRProj = proj)[[choice]]
            ))),
            paste0("colorset_", choice, ".csv")
          )
        }
        colorRampPalette(brewer.pal(12, "Paired"))(length(dcn()))
      }
  ...
  output$sc1a1sub3.ui <- renderUI(
    { # vecFeats = vector of feature names for colors
      colInput <- function(vecFeats) {
        pickers <- (lapply(1:length(vecFeats), function(k) {
          colourInput(
            inputId = paste0("col", k),
            label = dcn()[k], # color sel label for user
            if (
              file.exists(paste0("colorset_", input$sc1a1inp1, ".csv")) == TRUE
            ) {
              value = read.csv(
                paste0("colorset_", input$sc1a1inp1, ".csv")
              )$x[k]
            } else {
              value = defCol()[k]  # match initial plot
            },
  ...
  observeEvent(input$sc1a2togL, {
    lapply(1:length(dcn()), function(k) {
      updateColourInput(
        ...
        inputId = paste0("col", k),
        label = dcn()[k], # color sel label for user
        value = read.csv(paste0("colorset_", input$sc1a2inp1, ".csv"))$x[k],
        showColour = "both"  # show hex and color itself
      )
    })
  }, ignoreInit=TRUE
  ...
  output$sc2de1oup.pdf <- downloadHandler(
    filename = function() { paste0("sc2","_","volcano",".pdf") },
    content = function(file) { ggsave(
      file, device = "pdf", height = input$sc2de1oup.h, width = input$sc2de1oup.w, useDingbats = FALSE,
      plot = scvolcano_m(input$sc2de1grp) )
    })
  output$sc2de1oup.png <- downloadHandler(
    filename = function() { paste0("sc2","_","volcano",".png") },
    content = function(file) { ggsave(
  ...
  output$sc2a1oup1 <- renderPlot({
    scDRcell(sc2conf, sc2meta, input$sc2a1drX, input$sc2a1inp1,
  ...
  output$sc2a1oup1.pdf <- downloadHandler(
    ...
    content = function(file) { ggsave(
      ...
      plot = scDRcell(sc2conf, sc2meta, input$sc2a1drX, input$sc2a1inp1,
                      input$sc2a1sub1, input$sc2a1sub2,
                      input$sc2a1siz, input$sc2a1col1, input$sc2a1ord1,
                      input$sc2a1fsz, input$sc2a1asp, input$sc2a1txt, input$sc2a1lab1) )
    })
  ...
  output$sc2a2oup2.pdf <- downloadHandler(
    ...
    content = function(file) { ggsave(
      ...
      plot = scDRcell(sc2conf, sc2meta, input$sc2a2drX, input$sc2a2inp2,
                      input$sc2a2sub1, input$sc2a2sub2,
                      input$sc2a2siz, input$sc2a2col2, input$sc2a2ord2,
                      input$sc2a2fsz, input$sc2a2asp, input$sc2a2txt, input$sc2a2lab2) )
    })

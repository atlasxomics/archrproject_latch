# this version produced to use when the project has 3 conditions or more!
...
shinyUI(fluidPage(  
  ...
  navbarPage( 
        ...
        column(
          ...
          fluidPage(
            sidebarLayout(
              ...
              mainPanel = mainPanel( width = 6,uiOutput("sc1a1oup2.ui")
    ...
    tabPanel(
      ...
      # )  # End of column (6 space)
      "In this tab, users can visualise the volcano plot for marker gene differential accessibility",
      "Important Note: if one cluster shows more than 90 percent of one condition (check Proportion plot tab), volcano plot will not be calculated for that cluster!",
      ...
      fluidPage(
        sidebarLayout(
          sidebarPanel(
            ...
            downloadButton("sc1de1oup.png", "Download PNG"), br(),
            div(style="display:inline-block",
                numericInput("sc1de1oup.h", "PDF / PNG height:", width = "138px",
                             min = 4, max = 20, value = 8, step = 0.5)),
            div(style="display:inline-block",
                numericInput("sc1de1oup.w", "PDF / PNG width:", width = "138px",
                             min = 4, max = 20, value = 10, step = 0.5)),
            
          ), mainPanel = mainPanel(
            plotOutput('sc1de1oup')
          )# End of column (6 space)
          # fluidRow(column(6, plotOutput("sc1de1oup"))    
        ),  # End of column (6 space),
    ...
    tabPanel(
      ...
      # )  # End of column (6 space)
      "In this tab, users can visualise the volcano plot for marker motifs enrichments differentially",
      br(),br(),
      fluidPage(
        sidebarLayout(
          sidebarPanel(
            ...
            downloadButton("sc2de1oup.png", "Download PNG"), br(),
            div(style="display:inline-block",
                numericInput("sc2de1oup.h", "PDF / PNG height:", width = "138px",
                             min = 4, max = 20, value = 8, step = 0.5)),
            div(style="display:inline-block",
                numericInput("sc2de1oup.w", "PDF / PNG width:", width = "138px",
                             min = 4, max = 20, value = 10, step = 0.5)),
            ...
            plotOutput('sc2de1oup')
          )# End of column (6 space)
          # fluidRow(column(6, plotOutput("sc2de1oup"))    
        ),  # End of column (6 space),

...
shinyServer(function(input, output, session) { 
  ...
  output$sc1de1oup.pdf <- downloadHandler(
    filename = function() { paste0("sc1","_","volcano",".pdf") },
    content = function(file) { ggsave(
      file, device = "pdf", height = input$sc1de1oup.h, width = input$sc1de1oup.w, useDingbats = FALSE,
      plot = scvolcano(input$sc1de1grp) )
    })
  output$sc1de1oup.png <- downloadHandler(
    filename = function() { paste0("sc1","_","volcano",".png") },
    content = function(file) { ggsave(
  ...
  observeEvent(input$sc1c1togL, {
    ...
    lapply(1:length(dcn()), function(k) {
      updateColourInput(session,
                        inputId = paste0("col", k)
                        ,label = dcn()[k]      # color sel label for user
                        ,value = read.csv(paste0('colorset_',input$sc1c1inp1,'.csv'))$x[k] #  this should match initial plot
                        ,showColour = "both"  # show hex and color itself
                        # ,width = 2
      )})
    # print(read.csv(paste0('colorset_',input$sc1a2inp1,'.csv'))$x)
  ...
  dcn4 <- reactive({strsplit(sc1conf[UI == input$sc1c1inp1]$fID, "\\|")[[1]] })
  output$sc1c1oup <- renderPlot({ 
    scVioBox(sc1conf, sc1meta, input$sc1c1inp1, input$sc1c1inp2, 
             input$sc1c1sub1, input$sc1c1sub2, 
             paste0("input$col", 1:length(dcn4())) %>%   # use data color names
               map(., function(i) {eval(parse(text = i))}) %>% # convert strings to obj
               unlist() %>%  setNames(dcn4()),
  ...
  dcn5 <- reactive({strsplit(sc1conf[UI == input$sc1c2inp2]$fID, "\\|")[[1]] })
  output$sc1c2oup <- renderPlot({
    scProp(sc1conf, sc1meta, input$sc1c2inp1, input$sc1c2inp2,
           input$sc1c2sub1, input$sc1c2sub2,
           paste0("input$col", 1:length(dcn5())) %>%   # use data color names
             map(., function(i) {eval(parse(text = i))}) %>% # convert strings to obj
             unlist() %>%  setNames(dcn5()),
